    ...
    public function getTenantSchemaFrom($prepends = [], $appends = [], $excludes = [])
    {
        ...
        $contents = [
            ...
            PhoneNumber::make('whatsapp')
                ->label(__('tenant::tenant.forms.whatsapp.label'))
                ->placeholder(__('tenant::tenant.forms.whatsapp.placeholder'))
                ->columnSpan([
                    'md' => config('tenant.tenant.whatsapp.span', 4),
                ])
                ->required(config('tenant.tenant.whatsapp.required', false))
                ->maxLength(config('tenant.tenant.whatsapp.maxLength', 50)),
            Forms\Components\TextInput::make('contact_url')
                ->label(__('tenant::tenant.forms.contact_url.label'))
                ->placeholder(__('tenant::tenant.forms.contact_url.placeholder'))
                ->columnSpan([
                    'md' => config('tenant.tenant.contact_url.span', 4),
                ])
                ->required(config('tenant.tenant.contact_url.required', false))
                ->maxLength(config('tenant.tenant.contact_url.maxLength', 255)),
            Forms\Components\TextInput::make('redirect_url')
                ->label(__('tenant::tenant.forms.redirect_url.label'))
                ->placeholder(__('tenant::tenant.forms.redirect_url.placeholder'))
                ->columnSpan([
                    'md' => config('tenant.tenant.redirect_url.span', 4),
                ])
                ->required(config('tenant.tenant.redirect_url.required', false))
                ->maxLength(config('tenant.tenant.redirect_url.maxLength', 255)),
            Forms\Components\RichEditor::make('service_hours')

                ->label(__('tenant::tenant.forms.service_hours.label'))
                ->placeholder(__('tenant::tenant.forms.service_hours.placeholder'))
                ->columnSpan([
                    'md' => config('tenant.tenant.service_hours.span', 12),
                ])
                ->required(config('tenant.tenant.service_hours.required', false)),
            Forms\Components\Toggle::make('election_period')
                ->label(__('tenant::tenant.forms.election_period.label'))
                ->columnSpan([
                    'md' => config('tenant.tenant.election_period.span', 12),
                ])
                ->required(config('tenant.tenant.election_period.required', false)),
